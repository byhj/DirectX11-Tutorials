...
bool GraphicsClass::Init(int screenWidth, int screenHeight, HWND hWnd)
{
	...
	result = pModel->Init(pD3D->GetDevice(), "square.txt", L"../common/media/texture/stone01.dds", 
		                 L"../common/media/texture/dirt01.dds", L"../common/media/texture/alpha01.dds");
...
bool GraphicsClass::RenderGraphics()
{
	...
	result = pShader->Render(pD3D->GetDeviceContext(), pModel->GetIndexCount(), World, View, Proj, pModel->GetTextureArray() );
    if (!result)
	{
		return false;
	}
