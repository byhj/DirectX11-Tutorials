...
bool ShaderClass::Render(ID3D11DeviceContext *pD3D11DeviceContext, int IndexCount
						 ...
						 float specularPower, float fogStart, float fogEnd)
{
	bool result;
	result = SetShaderParameters(pD3D11DeviceContext, World, View, Proj, pTexture, 
		                         lightDirection, diffuseColor, cameraPosition, specularColor, 
								 specularPower, fogStart, fogEnd);
	if(!result)
	{
		return false;
	}
...
bool ShaderClass::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateBuffer(&fogBufferDesc, NULL, &pFogBuffer);
	if(FAILED(result))
	{
		return false;
	}
									  ...
									  float specularPower, float fogStart, float fogEnd)
{
	...
	result = pD3D11DeviceContext->Map(pFogBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	// Get a pointer to the data in the constant buffer.
	dataPtr4 = (FogBuffer*)mappedResource.pData;
