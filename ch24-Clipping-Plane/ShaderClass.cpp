						 ...
						 D3DXVECTOR4 clipPlane)
{
	...
	result = SetShaderParameters(pD3D11DeviceContext, World, View, Proj, pTexture, 
		                         lightDirection, diffuseColor, cameraPosition, specularColor, specularPower, clipPlane);
	if(!result)
	{
		return false;
	}
...
bool ShaderClass::SetShaderParameters(ID3D11DeviceContext* pD3D11DeviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
									  ...
									  float specularPower, D3DXVECTOR4 clipPlane)
{
	HRESULT result;
	...
	ClipPlaneBufferType* dataPtr4;
	// Lock the clip plane constant buffer so it can be written to.
	result = pD3D11DeviceContext->Map(m_clipPlaneBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	dataPtr4 = (ClipPlaneBufferType*)mappedResource.pData;

	// Copy the clip plane into the clip plane constant buffer.
	dataPtr4->clipPlane = clipPlane;

	// Unlock the buffer.
	pD3D11DeviceContext->Unmap(m_clipPlaneBuffer, 0);
	...
	pD3D11DeviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_clipPlaneBuffer);
	return true;
}
