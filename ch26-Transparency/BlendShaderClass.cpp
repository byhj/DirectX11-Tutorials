...
BlendShaderClass::BlendShaderClass()
	:pVS_Shader(0),
	 pPS_Shader(0),
	 pInputLayout(0),
	 pMatrixBuffer(0),
...
BlendShaderClass::BlendShaderClass(const BlendShaderClass &)
{
...
bool BlendShaderClass::Init(ID3D11Device *pD3D11Device, HWND hwnd)
{
	bool result;
...
void BlendShaderClass::Shutdown()
{
	ShutdownShader();
...
bool BlendShaderClass::Render(ID3D11DeviceContext *pD3D11DeviceContext, int IndexCount
						 ...
						 float translation)
{
	...
	result = SetShaderParameters(pD3D11DeviceContext, World, View, Proj, pTexture, 
		                         lightDirection, diffuseColor, cameraPosition, specularColor, specularPower, translation);
	if(!result)
	{
		return false;
	}
...
bool BlendShaderClass::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	translateBufferDesc.ByteWidth = sizeof(TransparencyBufferType);
...
void BlendShaderClass::ShutdownShader()
{
	if(pCameraBuffer)
	{
...
void BlendShaderClass::OutputShaderErrorMessage(ID3D10Blob *pErrorMessage, HWND hwnd, WCHAR *shaderFileName)
{
	char *pCompileErrors;
...
bool BlendShaderClass::SetShaderParameters(ID3D11DeviceContext* pD3D11DeviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
									  ...
									  float specularPower, float translation)
{
	HRESULT result;
	...
	TransparencyBufferType* dataPtr4;
	// Lock the clip plane constant buffer so it can be written to.
	result = pD3D11DeviceContext->Map(m_translateBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	dataPtr4 = (TransparencyBufferType*)mappedResource.pData;

	// Copy the clip plane into the clip plane constant buffer.
	dataPtr4->blendAmount = translation;

	// Unlock the buffer.
	pD3D11DeviceContext->Unmap(m_translateBuffer, 0);
...
void BlendShaderClass::RenderShader(ID3D11DeviceContext *pD3D11DeviceContext, int IndexCount)
{
