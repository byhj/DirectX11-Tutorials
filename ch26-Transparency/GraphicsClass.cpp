...
bool GraphicsClass::Init(int screenWidth, int screenHeight, HWND hWnd)
{
	...
	pCamera->SetPosition(0.0f, 0.0f, -6.0f);
	...
	pModel2 = new ModelClass;
	if (!pModel2)
	{
		return false;
	}

	result = pModel2->Init(pD3D->GetDevice(), "cube.txt", L"../common/media/texture/stone01.dds", 
	...
	//////////////////////////////////////////////////////////////////////////////
	pBlendShader = new BlendShaderClass;
	if (!pBlendShader )
	{
		return false;
	}

	result = pBlendShader->Init(pD3D->GetDevice(), hWnd);
	if(!result)
	{
		MessageBox(hWnd, L"Could not initialize the color shader object.", L"Error", MB_OK);
		return false;
...
bool GraphicsClass::RenderGraphics(float rot)
{
	...
	D3DXMatrixTranslation(&TranslateX, 1.0f, 0.0f, 0.0f);
	World *= TranslateX;

	pD3D->TurnOnAlphaBlending();
	//Render the Model Using Shader
	result = pBlendShader->Render(pD3D->GetDeviceContext(), pModel2->GetIndexCount(), World, View, Proj, 
		                     pModel2->GetTextureArray(), pLight->GetDirection(), pLight->GetDiffuseColor(),
							 pCamera->GetPosition(), pLight->GetSpecularColor(), pLight->GetSpecularPower(),blend);
   if (!result)
	{
		return false;
	}
