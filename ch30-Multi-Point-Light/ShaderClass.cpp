...
bool ShaderClass::Render(ID3D11DeviceContext *pD3D11DeviceContext, int IndexCount
						 ...
						 ID3D11ShaderResourceView* pTexture,D3DXVECTOR4 diffuseColor[],
						 D3DXVECTOR4 lightPosition[])
						
{
	bool result;
	result = SetShaderParameters(pD3D11DeviceContext, World, View, Proj, pTexture, diffuseColor, lightPosition);
...
bool ShaderClass::SetShaderParameters(ID3D11DeviceContext* pD3D11DeviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
									  D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView *pTexture, D3DXVECTOR4 diffuseColor[], D3DXVECTOR4 lightPosition[])
									
{
	...
	result = pD3D11DeviceContext->Map(m_lightPositionBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Unlock the constant buffer.
	pD3D11DeviceContext->Unmap(m_lightPositionBuffer, 0);

	// Set the position of the constant buffer in the vertex shader.
	bufferNumber = 1;

	// Finally set the constant buffer in the vertex shader with the updated values.
	pD3D11DeviceContext->VSSetConstantBuffers(bufferNumber, 1, &m_lightPositionBuffer);

	// Set shader texture resource in the pixel shader.
	pD3D11DeviceContext->PSSetShaderResources(0, 1, &pTexture);

		// Lock the light color constant buffer so it can be written to.
		result = pD3D11DeviceContext->Map(m_lightColorBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Unlock the constant buffer.
	pD3D11DeviceContext->Unmap(m_lightColorBuffer, 0);

	// Set the position of the constant buffer in the pixel shader.
	bufferNumber = 0;

	// Finally set the constant buffer in the pixel shader with the updated values.
	pD3D11DeviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_lightColorBuffer);
