...
bool FontShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitShader(device, hwnd, L"font-vs.hlsl", L"font-fs.hlsl");
	if(!result)
	{
		return false;
	}
...
bool FontShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateBuffer(&constantBufferDesc, NULL, &pMatrixBuffer);
	if(FAILED(result))
	{
		return false;
	}
...
void FontShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
	...
	deviceContext->VSSetShader(pVS_Shader, NULL, 0);
	deviceContext->PSSetShader(pPS_Shader, NULL, 0);
