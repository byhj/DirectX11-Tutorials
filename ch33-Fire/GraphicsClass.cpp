...
bool GraphicsClass::Init(int screenWidth, int screenHeight, HWND hWnd)
{
	...
	result = pModel->Init(pD3D->GetDevice(), L"../common/media/texture/fire01.dds", L"../common/media/texture/noise01.dds",
		L"../common/media/texture/alpha01.dds","square.txt");
...
bool GraphicsClass::RenderGraphics(float rot)
{
	...
	result = pShader->Render(pD3D->GetDeviceContext(), pModel->GetIndexCount(), World, View, Proj, 
		                   pModel->GetTexture1(), pModel->GetTexture2(), pModel->GetTexture3(), 
		                   frameTime, scrollSpeeds, scales, distortion1, distortion2,
