...
bool SystemClass::Render()
{
	...
	pPosition->SetFrameTime(pTimer->GetTime());
	// Check if the left or right arrow key has been pressed, if so rotate the camera accordingly.
	keyDown = pInput->IsLeftArrowPressed();
	pPosition->MoveLeft(keyDown);

	keyDown = pInput->IsRightArrowPressed();
	pPosition->MoveRight(keyDown);

	float positionX, positionY, positionZ;
	// Get the view point position/rotation.
	pPosition->GetPosition(positionX, positionY, positionZ);

	// Do the frame processing for the graphics object.
	result = pGraphics->Render(positionX, positionY, positionZ);
	if(!result)
	{
		return false;
	}
