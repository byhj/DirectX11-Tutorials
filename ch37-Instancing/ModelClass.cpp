...
bool ModelClass::InitBuffer(ID3D11Device *pD3D11Device)
{
	...
	InstanceData[0].position = D3DXVECTOR3(-1.5f, -1.5f, 5.0f);
	InstanceData[1].position = D3DXVECTOR3(-1.5f,  1.5f, 5.0f);
	InstanceData[2].position = D3DXVECTOR3( 1.5f, -1.5f, 5.0f);
	InstanceData[3].position = D3DXVECTOR3( 1.5f,  1.5f, 5.0f);
	...
	result = pD3D11Device->CreateBuffer(&instanceBufferDesc, &IntData, &pInstanceBuffer);
	if(FAILED(result))
	{
		return false;
	}
