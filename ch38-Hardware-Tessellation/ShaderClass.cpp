...
{
	bool result;
	result = InitShader(pD3D11Device, hwnd, L"color-vs.hlsl", L"color-hs.hlsl",
		                L"color-ds.hlsl", L"color-ps.hlsl");
...
bool ShaderClass::Render(ID3D11DeviceContext *pD3D11DeviceContext, int IndexCount
						 ,D3DXMATRIX World, D3DXMATRIX View, D3DXMATRIX Proj, float tessellationAmount )
{
	bool result;
	result = SetShaderParameters(pD3D11DeviceContext, World, View, Proj, tessellationAmount);
	if(!result)
	{
		return false;
	}
...
bool ShaderClass::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* hsFilename, WCHAR* dsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = device->CreateHullShader(hullShaderBuffer->GetBufferPointer(), hullShaderBuffer->GetBufferSize(), NULL, &pHS_Shader);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = device->CreateDomainShader(domainShaderBuffer->GetBufferPointer(), domainShaderBuffer->GetBufferSize(), NULL, &pDS_Shader);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = device->CreateBuffer(&tessellationBufferDesc, NULL, &pTessellationBuffer);
	if(FAILED(result))
	{
		return false;
	}
										   ...
										   D3DXMATRIX projectionMatrix, float tessellationAmount)
{
	...
	pD3D11DeviceContext->DSSetConstantBuffers(bufferNumber, 1, &pMatrixBuffer);



	TessellationBufferType* dataPtr2;
	// Lock the tessellation constant buffer so it can be written to.
	result = pD3D11DeviceContext->Map(pTessellationBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if(FAILED(result))
	{
		return false;
	}
	...
	pD3D11DeviceContext->Unmap(pTessellationBuffer, 0);
	...
	pD3D11DeviceContext->HSSetConstantBuffers(bufferNumber, 1, &pTessellationBuffer);
