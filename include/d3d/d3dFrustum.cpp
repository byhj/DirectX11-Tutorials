...
bool D3DFrustum::CheckCube(float xCenter, float yCenter, float zCenter, float radius)
{
	...
	for(int i=0; i<6; i++) 
	{
		if(D3DXPlaneDotCoord(&m_Planes[i], &XMFLOAT3((xCenter - radius), (yCenter - radius), (zCenter - radius))) >= 0.0f)
		{
			continue;
		}
...
bool D3DFrustum::CheckRectangle(float xCenter, float yCenter, float zCenter, float xSize, float ySize, float zSize)
{
	...
	for(int i=0; i<6; i++)
	{
		if(D3DXPlaneDotCoord(&m_Planes[i], &XMFLOAT3((xCenter - xSize), (yCenter - ySize), (zCenter - zSize))) >= 0.0f)
		{
			continue;
		}
