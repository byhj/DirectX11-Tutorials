...
public:
	...
	void Render(ID3D11DeviceContext *pD3D11DeviceContext, const XMFLOAT4X4 &Model,  
		const XMFLOAT4X4 &View, const XMFLOAT4X4 &Proj, const XMFLOAT4 &camPos);
				  ...
				  const XMFLOAT4X4 &View, const XMFLOAT4X4 &Proj, const XMFLOAT4 &camPos)
{
	...
	pD3D11DeviceContext->VSSetConstantBuffers(bufferSlot, 1, &m_pMVPBuffer);

	bufferSlot = 1;
	pD3D11DeviceContext->UpdateSubresource(m_pCameraBuffer, 0, NULL, &camPos, 0, 0 );
	pD3D11DeviceContext->VSSetConstantBuffers(bufferSlot, 1, &m_pCameraBuffer);
...
bool Cube::init_buffer(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext)
{
	...
	dataPtr2->ambientColor   = XMFLOAT4(0.65f, 0.65f, 0.65f, 0.15f);
	...
	dataPtr2->specularColor  = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	hr = pD3D11Device->CreateBuffer(&cameraBufferDesc, NULL, &m_pCameraBuffer);
	DebugHR(hr);
