...
void Cube::Render(ID3D11DeviceContext *pD3D11DeviceContext, const d3d::MatrixBuffer &matrix)
{
	...
	int bufferSlot = 1;
	pD3D11DeviceContext->UpdateSubresource(m_CameraBuffer.Get(), 0, NULL, &texTrans, 0, 0);
	pD3D11DeviceContext->PSSetConstantBuffers(bufferSlot, 1, m_CameraBuffer.GetAddressOf());
...
void Cube::init_buffer(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext)
{
	...
	dataPtr2->lightDirection = XMFLOAT3(.0f, 3.0f, 1.0f);
	...
	D3D11_BUFFER_DESC TexTransBufferDesc;
	TexTransBufferDesc.Usage               = D3D11_USAGE_DEFAULT;
	TexTransBufferDesc.ByteWidth           = sizeof(XMFLOAT4);
	TexTransBufferDesc.BindFlags           = D3D11_BIND_CONSTANT_BUFFER;
	TexTransBufferDesc.CPUAccessFlags      = 0;
	TexTransBufferDesc.MiscFlags           = 0;
	TexTransBufferDesc.StructureByteStride = 0;

	// Create the camera constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	hr = pD3D11Device->CreateBuffer(&TexTransBufferDesc, NULL, &m_CameraBuffer);
	//DebugHR(hr);
