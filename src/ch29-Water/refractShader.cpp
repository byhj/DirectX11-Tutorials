...
namespace byhj
{
	void RefractShader::Init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext, HWND hWnd)
	{
		...
		m_RefractShader.attachVS(L"light.vsh", "LightVS", "vs_5_0");
		m_RefractShader.attachPS(L"light.psh", "LightPS", "ps_5_0");
		m_RefractShader.end();
	...
	void RefractShader::Use(ID3D11DeviceContext *pD3D11DeviceContext, const d3d::MatrixBuffer &matrix)
	{
		cbMatrix = matrix;
