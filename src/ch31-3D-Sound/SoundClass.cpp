...
SoundClass::SoundClass(const SoundClass & rhs)
{
...
bool SoundClass::Init(HWND hWnd, char *soundFile)
{
	bool result ;
	...
	result = LoadWaveFile(soundFile, &pSecondaryBuffer, &m_secondary3DBuffer1);
	if (!result)
	{
		return false;
	}
...
{
	ShutdownWaveFile(&pSecondaryBuffer, &m_secondary3DBuffer1);
...
bool SoundClass::InitDirectSound(HWND hWnd)
{
	...
	result = pPrimaryBuffer->QueryInterface(IID_IDirectSound3DListener8, (LPVOID*)&m_listener);
	if(FAILED(result))
	{
		return false;
	}
