...
namespace byhj
{
	...
	void HorizontalShader::Init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext, HWND hWnd)
	{
		...
		horizontalShader.init(pD3D11Device, hWnd);
		horizontalShader.attachVS(L"horizontalblur.vsh", pInputLayoutDesc, numElements);
		horizontalShader.attachPS(L"horizontalblur.psh");
	...
	void HorizontalShader::Use(ID3D11DeviceContext *pD3D11DeviceContext, const byhj::MatrixBuffer &matrix)
	{
