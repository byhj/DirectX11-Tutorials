...
namespace byhj
{
	...
	void GlowMapShader::Init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext, HWND hWnd)
	{
		...
		glowMapShader.init(pD3D11Device, vInputLayoutDesc);
		glowMapShader.attachVS(L"glowMap.vsh", "GlowMapVS", "vs_5_0");
		glowMapShader.attachPS(L"glowMap.psh", "GlowMapPS", "ps_5_0");
		glowMapShader.end();
	...
	void GlowMapShader::Use(ID3D11DeviceContext *pD3D11DeviceContext, const byhj::d3d::MatrixBuffer &matrix,
				      ID3D11ShaderResourceView *pTexture1, ID3D11ShaderResourceView *pTexture2)
	{
		glowMapShader.use(pD3D11DeviceContext);
		...
		pD3D11DeviceContext->PSSetShaderResources(0, 1, &pTexture1);
		pD3D11DeviceContext->PSSetShaderResources(1, 1, &pTexture2);
	}
