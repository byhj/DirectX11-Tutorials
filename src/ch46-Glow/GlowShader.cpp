...
namespace byhj
{
	...
	void GlowShader::Init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext, HWND hWnd)
	{
		...
		glowShader.init(pD3D11Device, vInputLayoutDesc);
		glowShader.attachVS(L"glow.vsh", "GlowVS", "vs_5_0");
		glowShader.attachPS(L"glow.psh", "GlowPS", "ps_5_0");
		glowShader.end();
	...
	void GlowShader::Use(ID3D11DeviceContext *pD3D11DeviceContext, const byhj::d3d::MatrixBuffer &matrix,
				      ID3D11ShaderResourceView *pTexture1, ID3D11ShaderResourceView *pTexture2)
	{
		glowShader.use(pD3D11DeviceContext);
