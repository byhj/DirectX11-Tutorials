...
void D3DRenderSystem::init_object()
{
	...
	camera.init(m_ScreenWidth, m_ScreenHeight, cube.GetvPos(), cube.GetvIndex(), XMMatrixIdentity());
...
{
	bool pickFlag = camera.CheckPick();
	WCHAR WinInfo[255];
	...
	if (pickFlag)
	  swprintf(WinInfo, L"Picking: Yes");
	else
      swprintf(WinInfo, L"Picking: No");
	font.drawText(m_pD3D11DeviceContext, WinInfo, 22.0f, 10.0f, 100.0f);
}
