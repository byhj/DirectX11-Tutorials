...
private:
	...
	ID3D11RenderTargetView   *pRenderTextureSRVs[2];
	ID3D11ShaderResourceView *pShaderResourceView[2];
...
bool D3DRenderSystem::v_InitD3D()
{
	...
	result = m_pD3D11Device->CreateRenderTargetView(pRenderTargetTexture, &renderTargetViewDesc, &pRenderTextureSRVs[0]);
	result = m_pD3D11Device->CreateRenderTargetView(pRenderTargetTexture, &renderTargetViewDesc, &pRenderTextureSRVs[1]);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_pD3D11Device->CreateShaderResourceView(pRenderTargetTexture, &shaderResourceViewDesc, &pShaderResourceView[0]);
	result = m_pD3D11Device->CreateShaderResourceView(pRenderTargetTexture, &shaderResourceViewDesc, &pShaderResourceView[1]);
	if (FAILED(result))
	{
		return false;
	}
...
void D3DRenderSystem::v_Render()
{
	...
	m_pD3D11DeviceContext->ClearRenderTargetView(pRenderTextureSRVs[0], bgColor);
	m_pD3D11DeviceContext->ClearRenderTargetView(pRenderTextureSRVs[1], bgColor);
	...
	d3dRtt.Render(m_pD3D11DeviceContext, &pShaderResourceView[0], m_Model, m_View, orth);
...
void D3DRenderSystem::init_object()
{
	...
	d3dRtt.init_window(-1.0f, 1.0f, 2.0f, 2.0f, GetAspect() );
