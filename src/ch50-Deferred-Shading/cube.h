...
public:
	...
	std::vector<XMFLOAT3> GetvPos()
	{
		return PosData;
	}
	std::vector<unsigned long> GetvIndex()
	{
		return IndexData;
	}
private:
...
bool Cube::init_shader(ID3D11Device *pD3D11Device, HWND hWnd)
{
	...
	CubeShader.attachVS(L"deferred.vsh", pInputLayoutDesc, numElements);
	CubeShader.attachPS(L"deferred.psh");
	CubeShader.end();
